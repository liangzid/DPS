...
def load_dataset_data(
    ...
    if os.path.exists(pickle_save_path):
        ...
        read_path=f"k_dps__parallel_dataset__{dataset_name}/k_dps_output.json"
...
def create_contour_figure(dataset_names,inte_method="linear"):
    fig, axes = plt.subplots(1, len(dataset_names), figsize=(16, 4))
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        _, embedds, kdps = load_dataset_data(dataset_name)
       
        if embedds is not None:
            im = create_contour_subplot(axes[i], embedds, kdps, dataset_name,
    ...
    if inte_method=="linear":
        plt.savefig('kdps_contour_plots_linear.pdf',bbox_inches='tight')
    elif inte_method=="nearest":
        plt.savefig('kdps_contour_plots_nearest.pdf',bbox_inches='tight')
    else:
        plt.savefig('kdps_contour_plots.pdf',bbox_inches='tight')
    plt.close()
    print("Saved. Save to: kdps_contour_plots.pdf")

def create_heatmap_figure(dataset_names):
    fig, axes = plt.subplots(1, len(dataset_names), figsize=(16, 4))
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        if embedds is not None:
            im = create_heatmap_subplot(axes[i], embedds, kdps, dataset_name)
    ...
    plt.tight_layout(rect=[0, 0, 0.9, 0.95])
    plt.savefig('kdps_heatmap_plots.pdf', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved. Save to: kdps_heatmap_plots.pdf")

def create_3d_figure(dataset_names,inte_method="linear"):
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        if embedds is not None:
            ...
            im = create_3d_subplot(ax, embedds, kdps, dataset_name,inte_method=inte_method)
    ...
    plt.tight_layout(rect=[0, 0, 0.9, 0.95])
    plt.savefig('kdps_3d_plots.pdf', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved. Save to: kdps_3d_plots.pdf")

def visualize_all_separate(dataset_names):
    create_contour_figure(dataset_names,inte_method="linear")
    create_contour_figure(dataset_names,inte_method="nearest")
    create_contour_figure(dataset_names,inte_method="cubic")
    create_heatmap_figure(dataset_names)
    create_3d_figure(dataset_names)

if __name__ == "__main__":
    datasets = ["wikipedia_mini", "tulu-3-sft", "openo1-sft", "hh-rlhf"]
    visualize_all_separate(datasets)
